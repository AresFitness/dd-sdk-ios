...
internal class TracingURLSessionHandler: DatadogURLSessionHandler {
    ...
    var distributedTraceSampler: Sampling?
    ...
    func modify(request: URLRequest, headerTypes: Set<TracingHeaderType>, networkContext: NetworkContext?) -> (URLRequest, TraceContext?) {
        ...
        let sampler = sampler(defaultID: traceID.idLo)
    ...
    func interceptionDidComplete(interception: DatadogInternal.URLSessionTaskInterception) {
        ...
        if let trace = interception.trace {
            ...
            if distributedTraceSampler == nil {
                distributedTraceSampler = sampler(defaultID: defaultTraceID.idLo)
            }
    ...
    private func sampler(defaultID: UInt64) -> Sampling {
